...
# --- Page Config ---
st.set_page_config(page_title="AGV Path Precision", layout="wide")
...
if s_charge >= m_charge:
    active_unit, active_id, active_color, unit_label, current_soc = "S", "slave_agv", "#FF5252", "Slave", s_charge
else:
    active_unit, active_id, active_color, unit_label, current_soc = "M", "master_agv", "#4A90E2", "Master", m_charge

if 'execute_nav' not in st.session_state:
    st.session_state.execute_nav = False

# Mapping Racks to your CUSTOM line coordinates
# We align the 'y' movement to the exact center of your red and blue horizontal lines
PATH_CONFIG = {
    "L3": {"line_y": 45,  "x_target": 80},  # Matches Red Line 1
    "R3": {"line_y": 45,  "x_target": 420}, 
    "L2": {"line_y": 175, "x_target": 80},  # Middle ground between Red/Blue paths
    "R2": {"line_y": 175, "x_target": 420}, 
    "L1": {"line_y": 315, "x_target": 80},  # Matches Blue Line 2
    "R1": {"line_y": 315, "x_target": 420}
}
...
with col1:
    ...
    rack_choice = st.selectbox("Product Rack", list(PATH_CONFIG.keys()), index=3)
    ...
    st.markdown(f'<div class="op-status"><b>Operation Status :</b><br><br>AGV under performance : {unit_label}<br>State of Charge : {current_soc}%</div>', unsafe_allow_html=True)

with col2:
    conf = PATH_CONFIG[rack_choice]
    start_y = 480
    start_x = 200 if active_unit == "S" else 300
    
    # Precise calculation to land ON your defined lines
    dist_y = conf['line_y'] - start_y
    dist_x = conf['x_target'] - start_x
    ...
    <div style="background: white; border-radius: 10px; padding: 10px;">
        <svg width="100%" height="650" viewBox="0 0 500 650">
            ...
            <g fill="white" stroke="grey" font-family="Arial">
                <rect x="50" y="25" width="60" height="40" /><text x="65" y="50" stroke="none" fill="black">L3</text>
                <rect x="390" y="25" width="60" height="40" /><text x="405" y="50" stroke="none" fill="black">R3</text>
                <rect x="50" y="155" width="60" height="40" /><text x="65" y="180" stroke="none" fill="black">L2</text>
                <rect x="390" y="155" width="60" height="40" /><text x="405" y="180" stroke="none" fill="black">R2</text>
                <rect x="50" y="295" width="60" height="40" /><text x="65" y="320" stroke="none" fill="black">L1</text>
                <rect x="390" y="295" width="60" height="40" /><text x="405" y="320" stroke="none" fill="black">R1</text>
            </g>
        ...
        <script>
            if ({nav_trigger}) {{
                ...
                requestAnimationFrame(() => {{
                    requestAnimationFrame(() => {{
                        // Phase 1: Travel UP your defined vertical track
                        unit.style.transform = "translateY({dist_y}px)";
                        
                        // Phase 2: Turn and travel ON your horizontal line
                        setTimeout(() => {{
                            unit.style.transform = "translateY({dist_y}px) translateX({dist_x}px)";
                        }}, 1300);
                        
                        // Phase 3: Return to main vertical track
                        setTimeout(() => {{
                            unit.style.transform = "translateY({dist_y}px) translateX(0px)";
                        }}, 4000);
                        
                        // Phase 4: Return home to Charging Dock
                        setTimeout(() => {{
