...
with col2:
    # Build the SVG Warehouse Layout with Colored Horizontal Paths
    svg_html = f"""
    <div style="display: flex; justify-content: center;">
        <svg width="500" height="630" viewBox="0 0 500 630" xmlns="http://www.w3.org/2000/svg" style="background: white; border: 1px solid #ccc;">
            ...
            <line x1="200" y1="50" x2="200" y2="520" stroke="#FF5252" stroke-width="3" /> <line x1="300" y1="50" x2="300" y2="520" stroke="#4A90E2" stroke-width="3" /> <line x1="70" y1="90" x2="300" y2="90" stroke="#FF5252" stroke-width="2" opacity="0.6" />
            <line x1="70" y1="220" x2="300" y2="220" stroke="#FF5252" stroke-width="2" opacity="0.6" />
            <line x1="70" y1="350" x2="300" y2="350" stroke="#FF5252" stroke-width="2" opacity="0.6" />

            <line x1="200" y1="130" x2="430" y2="130" stroke="#4A90E2" stroke-width="2" opacity="0.6" />
            <line x1="200" y1="260" x2="430" y2="260" stroke="#4A90E2" stroke-width="2" opacity="0.6" />
            <line x1="200" y1="390" x2="430" y2="390" stroke="#4A90E2" stroke-width="2" opacity="0.6" />
...
if start_btn and not st.session_state.is_moving:
    ...
    # 1. TRAVEL TO RACK (UP)
    if is_slave:
        current_y = st.session_state.s_y
        while current_y > target_y:
            current_y -= 10
            st.session_state.s_y = current_y
            time.sleep(0.05)
            st.rerun()
    else:
        current_y = st.session_state.m_y
        while current_y > target_y:
            current_y -= 10
            st.session_state.m_y = current_y
            time.sleep(0.05)
            st.rerun()
    ...
    if is_slave:
        while st.session_state.s_y < 450:
            st.session_state.s_y += 10
            time.sleep(0.05)
            st.rerun()
    else:
        while st.session_state.m_y < 450:
            st.session_state.m_y += 10
            time.sleep(0.05)
            st.rerun()
